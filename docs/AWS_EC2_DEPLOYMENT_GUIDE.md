...
> ⚠️ **Lưu ý**: Tuyệt đối **không** chọn gói *Ubuntu Pro* để tránh phát sinh chi phí bản quyền ngoài ý muốn.
...
*(Tùy chọn: Nếu muốn mở trang quản lý RabbitMQ Management, thêm Port `15672` với Source là **My IP**).*
...
free -h
```
*(Lệnh `free -h` hiện dòng Swap `4.0Gi` là thành công).*
...
```
*(Thấy log báo `Nest application successfully started` là chuẩn).*
...
| :--- | :--- |
| **Xem log realtime** | `docker compose logs -f <service-name>` *(vd: `docker compose logs -f api-gateway`)* |
...
* **Cách khắc phục**:
  ...
  *(Địa chỉ IP này sẽ trở thành IP tĩnh vĩnh viễn, không bao giờ thay đổi).*
