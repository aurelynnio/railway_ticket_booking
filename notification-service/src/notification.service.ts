...
export class NotificationService {
  ...
  constructor(private readonly prisma: PrismaClient) {
    ...
    if (smtpHost && smtpUser && smtpPass) {
      ...
      this.logger.log(`Nodemailer transporter initialized for host: ${smtpHost}`);
    } else {
      this.logger.warn('SMTP settings are missing. Emails will be logged to console instead.');
    }
  }
  ...
  private async sendAndPersist(
    ...
    if (this.transporter) {
      try {
        ...
        this.logger.log(`Email sent successfully to ${data.email} with subject: "${subject}"`);
      } catch (error) {
        status = 'failed';
...
`);
    ...
    try {
      ...
      this.logger.error(`Failed to persist notification for ${data.email}:`, error);
    }
  }

  async handleUserRegistered(data: { userId?: string; email: string; fullName: string }) {
    ...
    await this.sendAndPersist({ userId: data.userId, email: data.email, type: 'user_registered' }, subject, text);
  }

  async handlePasswordReset(data: { userId?: string; email: string; token: string }) {
    ...
    await this.sendAndPersist({ userId: data.userId, email: data.email, type: 'password_reset' }, subject, text);
  }

  async handleOrderCreated(data: { userId?: string; email: string; orderId: string; totalPrice: number; trainNumber: string; seatLabels: string[] }) {
    ...
    await this.sendAndPersist({ userId: data.userId, email: data.email, type: 'order_created' }, subject, text);
  }

  async handlePaymentPaid(data: { userId?: string; email: string; orderId: string; amount: number; ticketCode: string }) {
    ...
    await this.sendAndPersist({ userId: data.userId, email: data.email, type: 'payment_paid' }, subject, text);
  }
}
